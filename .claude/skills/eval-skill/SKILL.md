...
Not an evaluation system. A safe runner and an honesty gate, until `claude plugin eval` opens and takes both jobs.

1. Try `claude plugin eval . --case "$name*" --runs 5`. If it runs, read `evals/results/` and stop here — it has ablation and scoring this does not.
...
5. **Baseline once:** same case, no `--plugin-dir`. A bare run that passes the rubric means the rubric is not measuring the skill.
