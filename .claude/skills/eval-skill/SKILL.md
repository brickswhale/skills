...
5. **Baseline once:** same case, no `--plugin-dir`, and `Skill` in `--disallowedTools` — six of these skills have a retired `kit-*` ancestor still installed, and one answered in its skill's place. A bare run that passes the rubric means the rubric is not measuring the skill.
