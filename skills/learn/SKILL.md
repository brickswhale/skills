...
A lesson that lives where nothing reads it is not a lesson.

1. **Start from the class.** One occurrence is an anecdote. Name the class, count it with a command, report the number. Count of one: say so and stop. No control warranted: give the evidence and stop.
2. **Run every control that already covers this class; paste what each printed.** One you cannot show running is unloaded — a loading failure, not a missing one: raise it to a rung nothing can skip, never duplicate it. Then take the highest rung that can catch the class. Each lower rung needs something to choose to read it, so it is weaker:
...
4. **Write the rule, not the story.** Say what to do, in the words of the moment it applies.
5. **One rule per class.** Two classes are two passes.
6. **Report** the class, its count, the rung taken, the rungs ruled out, and the change: replacement, addition, or loading fix.
