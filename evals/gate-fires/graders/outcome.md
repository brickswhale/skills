...
The callers box was reported at 2/5 and rewritten to name its target mechanically and state what a count of one means. Re-measured, it fired 4/5 before the rewrite and 4/5 after: the low number came from the pattern used to detect it, not from the skill, and the rewrite was reverted. Other boxes moved by one run in the same comparison with nothing changed. At five runs a one-run difference is noise, and this fixture's boxes sit around 80 percent. Do not tune a line here on a difference of one or two runs.
